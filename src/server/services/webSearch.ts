...
function classifyDomain(domain: string): { type: WebSourceType; score: number } {
  ...
  if (d.includes('reuters.com') || d.includes('bloomberg.com') || d.includes('bbc.com') || d.includes('thaipbs.or.th') || d.includes('thairath.co.th') || d.includes('thestandard.co') || d.includes('bangkokpost.com') || d.includes('matichon.co.th') || d.includes('prachachat.net')) return { type: 'news', score: 0.90 };
...
async function searchWikipedia(query: string): Promise<WebSearchResultItem[]> {
  ...
  await Promise.all(['th', 'en'].map(async (lang) => {
    try {
      const raw = await fetchText(`https://${lang}.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(query)}&limit=3&namespace=0&format=json&origin=*`, 5000);
      if (!raw) return;
      const data = JSON.parse(raw);
...
/**
 * Build a citation-safe evidence block for the downstream model.
 * The model receives an explicit source map so a bare [Source N] can never be
 * interpreted as a complete citation. Final user-facing citations MUST be
 * clickable Markdown links using only these exact URLs.
 */
export function formatWebSearchResultsForPrompt(searchExecution: WebSearchExecutionResult): string {
  ...
  const sourceIndex = searchExecution.results.map((result, index) =>
