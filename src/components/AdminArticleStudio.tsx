...
export const AdminArticleStudio: React.FC = () => {
  ...
  const startEdit = (article: any) => {
    ...
    setPublicUrl('');
    setStatus('กำลังแก้ไขบทความ — ฟอร์มแก้ไขอยู่ด้านล่าง');
    window.setTimeout(() => document.getElementById('admin-article-editor')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50);
  };
