...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onOfflineMode }) => {
  ...
  const handleEnterpriseSso = async () => {
    if (!enterpriseOidcProviderId) return;
    setError(null);
    setSuccessMessage(null);
    setIsLoading(true);
    try {
      const provider = new OAuthProvider(enterpriseOidcProviderId);
      const credential = await signInWithPopup(auth, provider);
      recordUserLogin({ uid: credential.user.uid, email: credential.user.email }).catch(() => {});
      setSuccessMessage('เข้าสู่ระบบองค์กรสำเร็จ');
      setTimeout(onClose, 1000);
    } catch (err: any) {
      const code = String(err?.code || 'unknown');
      setError(code === 'auth/operation-not-allowed' ? 'ผู้ดูแลยังไม่ได้เปิด Enterprise OIDC provider ใน Firebase Authentication' : `Enterprise SSO ล้มเหลว (${code})`);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {currentUser ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {enterpriseOidcProviderId && (
                <button type="button" onClick={handleEnterpriseSso} disabled={isLoading} className="w-full py-2.5 px-4 rounded-xl bg-sky-500/10 hover:bg-sky-500/20 text-sky-200 border border-sky-400/30 font-semibold text-xs transition-all flex items-center justify-center space-x-2 cursor-pointer disabled:opacity-50">
              ...
              <button
                ...
                className="w-full py-2.5 px-4 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 font-semibold text-xs transition-all flex items-center justify-center space-x-2 cursor-pointer shadow-sm"
              >
