...
import { Clipboard, Check, ShieldCheck, ExternalLink, ArrowLeft, RotateCcw, AlertTriangle } from 'lucide-react';
    ...
    <section className="max-w-6xl mx-auto w-full px-3 sm:px-5 py-4 space-y-4">
      <div className={`rounded-2xl border p-5 ${panel}`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><div className="flex items-center gap-2 text-amber-400 font-mono text-xs font-bold uppercase tracking-wider"><ShieldCheck className="w-4 h-4" /> Firekeeper AI Passport Companion</div><h1 className="text-xl sm:text-2xl font-bold mt-2">AI Governance Layer</h1><p className={`text-sm mt-1 max-w-3xl ${muted}`}>สร้าง governed prompt → ใช้ AI ผ่าน AiPASS หรือ provider โดยตรง → นำคำตอบกลับมาตรวจสอบใน Firekeeper</p></div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <div className={`rounded-2xl border p-5 ${panel}`}>
          ...
          <select value={provider} onChange={e => { setProvider(e.target.value as AIPassportProvider); setVerification(null); }} className={`mt-2 w-full rounded-xl border p-3 text-sm ${input}`}>{providers.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}</select>
      ...
      <div className={`rounded-2xl border p-5 ${panel}`}>
        <div className="flex items-center justify-between"><div><label className="text-xs font-mono font-bold uppercase tracking-wider">3. Paste AI Response</label><p className={`text-xs mt-1 ${muted}`}>คำตอบนี้เป็น verification input เท่านั้น ไม่ถือว่าเป็นหลักฐานโดยอัตโนมัติ</p></div><ExternalLink className="w-4 h-4 text-slate-500" /></div>
        <textarea value={response} onChange={e => { setResponse(e.target.value); setVerification(null); }} rows={8} placeholder="วางคำตอบจาก ChatGPT / Claude / Gemini / AiPASS ที่นี่..." className={`mt-3 w-full rounded-xl border p-3 text-sm outline-none focus:border-amber-500 ${input}`} />
        <button disabled={!question.trim() || !response.trim()} onClick={verify} className="mt-3 w-full sm:w-auto px-5 py-2.5 rounded-xl bg-amber-500 text-slate-950 font-bold text-sm disabled:opacity-40">Verify AI Response</button>
      </div>

      {verification && <div className={`rounded-2xl border p-5 ${panel}`}>
        <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-mono font-bold uppercase tracking-wider">4. Verification Result</div><div className={`text-3xl font-black mt-1 ${decisionClass}`}>{verification.score}/100</div></div><div className={`px-3 py-2 rounded-xl border text-xs font-mono font-bold ${decisionClass}`}>{verification.decision}</div></div>
        <div className="grid sm:grid-cols-4 gap-2 mt-4">{Object.entries(verification.checks).map(([key, value]) => <div key={key} className={`rounded-xl border p-3 text-xs ${value ? 'border-emerald-500/20' : 'border-amber-500/20'}`}><div className="font-mono text-slate-500">{key.replaceAll('_',' ').toUpperCase()}</div><div className="font-bold mt-1">{value ? 'PASS' : 'CHECK'}</div></div>)}</div>
        ...
        <div className={`mt-4 text-[11px] ${muted}`}>คะแนนนี้เป็น first-pass governance signal ไม่ใช่การพิสูจน์ความจริงของคำตอบหรือความถูกต้องของแหล่งอ้างอิง</div>
      </div>}

      <div className="grid sm:grid-cols-5 gap-2 text-center text-[10px] font-mono">{['INPUT','AI OUTPUT','EVIDENCE','VERIFY','HUMAN DECIDES'].map((x,i) => <div key={x} className={`rounded-lg border p-2 ${i <= (verification ? 3 : 1) ? 'border-amber-500/30 text-amber-400' : 'border-white/10 text-slate-500'}`}>{i+1}. {x}</div>)}</div>
      <div className={`rounded-xl border p-3 text-[11px] ${muted}`}><strong className="text-amber-400">Integration boundary:</strong> โหมดนี้ไม่เรียก API, token หรือ endpoint ภายใน AiPASS และไม่ทำ automation ของ AiPASS โดยตรง เป็น workflow แบบ user-mediated</div>
    </section>
  );
}
