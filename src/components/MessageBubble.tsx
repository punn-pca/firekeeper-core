  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isUser && (
          <div className={`mt-4 pt-3 border-t flex flex-wrap items-center justify-between gap-2 text-xs w-full ${
            ...
            <div className="flex items-center flex-wrap gap-2 text-[11px] font-mono">
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border font-medium ${
                ...
                🛡️ Human Agency: Advisory Only
              </span>
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-[10.5px] ${
              ...
              {displayUserQuestionTime && displayTime ? (
                <span 
                  className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded border text-[10.5px] ${
                    isLight ? 'bg-slate-100 text-slate-700 border-slate-300' : 'bg-slate-800/80 text-slate-300 border-slate-700/70'
                  }`}
                  title={`ถามเมื่อ: ${fullUserQuestionTime} | ตอบเมื่อ: ${fullDateTime}`}
                >
                  <Clock className={`w-3 h-3 ${isLight ? 'text-amber-600' : 'text-amber-400'}`} />
                  <span>ถาม: {displayUserQuestionTime} ➔ ตอบ: {displayTime}</span>
                  {formattedDuration && (
                    <span className={`font-semibold border-l pl-1.5 ml-0.5 ${
                      isLight ? 'text-emerald-700 border-slate-300' : 'text-emerald-400 border-slate-700'
                    }`}>
                      ({formattedDuration})
                    </span>
                  )}
                ...
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-[10.5px] ${
                  isLight ? 'bg-slate-100 text-slate-600 border-slate-200' : 'bg-slate-800/70 text-slate-400 border-slate-700/60'
                }`}>
                  <Clock className="w-3 h-3 text-slate-400" />
                  <span>ตอบเมื่อ {displayTime}</span>
                </span>
            ...
            <div className="flex items-center space-x-2 flex-wrap gap-y-1.5" data-export-ignore="true">
              <button
                type="button"
                data-export-ignore="true"
                onClick={() => setIsTraceModalOpen(true)}
                title="เปิดดู Execution Trace & Decision Audit Trail 10 ขั้นตอนตามมาตรฐาน PUNN PCA"
                className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg border transition-all text-xs cursor-pointer font-medium ${
                  isLight 
                    ? 'bg-amber-50 hover:bg-amber-100 text-amber-900 hover:text-amber-950 border-amber-300 shadow-xs' 
                    : 'bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 hover:text-amber-200 border-amber-500/40 shadow-xs'
                }`}
              >
              ...
              <button
                ...
                title="ส่งออกข้อความนี้เป็นไฟล์ HTML (DOM Snapshot)"
                className={`flex items-center space-x-1 px-3 py-1.5 rounded-lg border transition-all text-xs cursor-pointer ${
                  isLight 
                    ? 'bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-amber-700 border-slate-300' 
