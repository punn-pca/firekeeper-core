          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-500/20">
                {evidenceSufficiencyStatus || confidence.verificationState || (confidence.calibrationStatus === 'EMPIRICAL_VERIFIED' ? 'VERIFIED' : 'UNVERIFIED')}
              </span>
      ...
      <div className="p-3 sm:p-3.5 space-y-2.5 text-xs">
          ...
          <div className="space-y-1.5">
            ...
            {confidence.mathematicalProof && (
              <div className={`px-2.5 py-1.5 rounded-lg border font-mono text-[11px] flex items-center gap-2 ${isLight ? 'bg-emerald-50/70 border-emerald-200 text-emerald-900' : 'bg-emerald-950/30 border-emerald-800/40 text-emerald-300'}`}>
                <span className="font-bold text-[10px] uppercase px-1 py-0.2 rounded bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 shrink-0">Math Trace</span>
                <span className="truncate" title={confidence.mathematicalProof}>{confidence.mathematicalProof}</span>
              </div>
            )}
            {confidence.epistemicQuarantineActive && confidence.quarantineReason && (
              <div className={`px-2.5 py-1.5 rounded-lg border font-mono text-[11px] flex items-center gap-2 ${isLight ? 'bg-amber-50/80 border-amber-200 text-amber-900' : 'bg-amber-950/30 border-amber-800/40 text-amber-300'}`}>
                <span className="font-bold text-[10px] uppercase px-1 py-0.2 rounded bg-amber-500/20 text-amber-700 dark:text-amber-300 shrink-0">Quarantine</span>
                <span className="truncate" title={confidence.quarantineReason}>{confidence.quarantineReason}</span>
              </div>
            )}
          </div>
        )}
        ...
        <div className="metrics-scroll overflow-x-auto pb-1.5 -mx-0.5 px-0.5 touch-pan-x">
          <div className="metrics-grid">
            ...
            <div className={`p-2.5 rounded-lg border flex flex-col justify-between min-h-[72px] min-w-[110px] ${isLight ? 'bg-white/70 border-slate-200 shadow-xs' : 'bg-slate-950/40 border-slate-800/80 shadow-xs'}`}>
              ...
              <span className={`font-bold text-xs mt-1.5 font-mono truncate ${(confidence.verificationStatus || confidence.calibrationStatus) === 'VERIFIED' || (confidence.verificationStatus || confidence.calibrationStatus) === 'EMPIRICAL_VERIFIED' ? 'text-emerald-600 dark:text-emerald-400' : (confidence.verificationStatus || confidence.calibrationStatus) === 'PARTIALLY_VERIFIED' ? 'text-amber-600 dark:text-amber-400' : 'text-sky-600 dark:text-sky-400'}`} title={evidenceSufficiencyStatus || confidence.verificationStatus || confidence.calibrationStatus || 'NOT_VERIFIED'}>
                {evidenceSufficiencyStatus || confidence.verificationStatus || confidence.calibrationStatus || 'NOT_VERIFIED'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className={`p-2.5 rounded-lg border text-[11px] leading-relaxed flex items-start gap-2 ${isLight ? 'bg-sky-100/60 border-sky-300/70 text-sky-950' : 'bg-sky-950/40 border-sky-800/40 text-sky-200/90'}`}>
          ...
          <div className="space-y-1">
            <span className="font-bold font-mono text-[10px] uppercase px-1.5 py-0.2 rounded bg-sky-500/20 text-sky-700 dark:text-sky-300">Statistical Evidence Invariant</span>
            <p className="text-[11px]">{confidence.empiricalCalibrationNote || 'Strict Evidence Boundary Calibration: ความมั่นใจยึดโยงกับพยานหลักฐานเชิงประจักษ์ที่ผ่านการตรวจสอบ และถูกตัดคะแนนตามข้อจำกัดด้านสารสนเทศที่ไม่ครบถ้วน'}</p>
          </div>
        </div>
      </div>
    </div>
