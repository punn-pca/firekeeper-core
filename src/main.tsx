...
function extractGovernedPrompt(content: string): unknown {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && (parsed as any).mode === 'GOVERNED_PROMPT') return parsed;
  } catch {}
  return null;
}
...
function installChatJsonDownload() {
  ...
  const downloadTurnJson = (turnElement: HTMLElement) => {
    ...
    // For GOVERNED_PROMPT, export the prompt package itself as the JSON document.
    // Do not wrap it in another chat-export envelope.
